...
<table class="tableLists">
	...
	foreach($table_fetch as $tb): 
		$colums = new Database("mysql", array(
			"method"=>"colums",
			"tablename"=>$tb["Tables_in_atori_1"]
		));
	...
	?>
	<tr>
		<td><?=$tb["Tables_in_atori_1"]?></td>
		<td>
			<a href="javascript:void(0)" onclick="select('<?=$tb["Tables_in_atori_1"]?>')">Select</a>
			<a href="javascript:void(0)" onclick="update('<?=$tb["Tables_in_atori_1"]?>', '<?=implode(",", $fields)?>')">Update</a>
			<a href="javascript:void(0)" onclick="delet('<?=$tb["Tables_in_atori_1"]?>')">Delete</a>
		</td>
	</tr>
	<?php endforeach; ?>
</table>
