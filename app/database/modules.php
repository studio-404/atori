...
class modules
{
	...
	private function countAvaliableFloors($args)
	{
		$out = 0;
		$select = "SELECT COUNT(`id`) AS avaliable FROM `usefull` WHERE `type`=:type AND `additional2`=:one AND `lang`=:lang AND `status`!=:one";
		$prepare = $this->conn->prepare($select);
		$prepare->execute(array(
			":type"=>$args['type'], 
			":one"=>1,
	...
	private function selectById($args)
	{
		...
		$select = "SELECT *, (
				...
				`photos`.`status`!=:one 
				ORDER BY `photos`.`id` ASC LIMIT 1
		) AS photo FROM `usefull` WHERE `idx`=:idx AND `lang`=:lang AND `status`!=:one";
		$prepare = $this->conn->prepare($select); 
		$prepare->execute(array(
			":idx"=>$args['idx'], 
			":lang"=>$args['lang'], 
	...
	private function addParentFromPageAdd($args)
	{
		$type = $args['type'];
		$title = $args['title'];
	...
	private function selectMaxId($args){
