...
class page
{
	...
	private function selectById($args)
	{
		...
		$select = "SELECT *, (
			...
			) AS photo FROM `navigation` WHERE `idx`=:idx AND `lang`=:lang AND `status`!=:one";
		$prepare = $this->conn->prepare($select);
		$prepare->execute(array(
			":idx"=>$idx, 
			":lang"=>$lang, 
	...
	private function add($args)
	{
		...
		$terms = $args["terms"];
		$map = $args["map"];
		$additional1 = $args["additional1"];
		$additional2 = $args["additional2"];
		$additional3 = $args["additional3"];
		$additional4 = $args["additional4"];
		$additional5 = $args["additional5"];
		$additional6 = $args["additional6"];
		...
		foreach ($fetch as $val) {
			...
			`position`=:position,
			`visibility`=:visibility, 
			`aditional1`=:additional1, 
			`aditional2`=:additional2, 
			`aditional3`=:additional3, 
			`aditional4`=:additional4, 
			`aditional5`=:additional5, 
			`aditional6`=:additional6, 
			`status`=:status";
	...
	private function edit($args)
	{
		...
		$terms = $args["terms"];
		$map = $args["map"];
		$aditional1 = $args["aditional1"];
		$aditional2 = $args["aditional2"];
		$aditional3 = $args["aditional3"];
		$aditional4 = $args["aditional4"];
		$aditional5 = $args["aditional5"];
		$aditional6 = $args["aditional6"];
		...
		$selectType = "SELECT `type` FROM `navigation` WHERE `idx`=:idx AND `lang`=:lang";
		$prepareType = $this->conn->prepare($selectType);
		$prepareType->execute(array(
			":idx"=>$idx, 
			":lang"=>$lang
		));
		$fetchType = $prepareType->fetch(PDO::FETCH_ASSOC);
		$type = $fetchType["type"];
	...
	private function changeAditionalCols($args){
		$update = "UPDATE `navigation` SET `aditionaljson`=:aditionaljson WHERE `idx`=:idx AND `lang`=:lang";
		$prepare = $this->conn->prepare($update);
		$prepare->execute(array(
			":aditionaljson"=>$args["aditionaljson"], 
			":idx"=>$args["idx"], 
			":lang"=>$args["lang"]
		)); 
