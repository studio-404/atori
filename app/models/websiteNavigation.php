...
class websiteNavigation
{
	...
	public function index(){
		...
		$RESTRICTED_PAGE_DELETE_IDX = explode("|", Config::RESTRICTED_PAGE_DELETE_IDX);

		
		$string = new functions\string();
		$aditionalLink = "/".$_SESSION["LANG"]."/dashboard/plugins/navigation/";
		$nav = "";
			...
			foreach ($this->navigation as $val)
			{
				...
				$nav .= sprintf("
					<div class=\"row level-0\" data-item=\"%d\" data-cid=\"%d\" data-level=\"0\" style=\"position:relative\">
						...
						<a href=\"javascript:void(0)\" onclick=\"changeVisibility('%s','%s')\">
					 	...
					 	%s%s
						<a href=\"javascript:void(0)\" onclick=\"askRemovePage('0','%s','%s','%s')\" ".$delHide.">
				...
				if($subNavigation->getter()){
					foreach ($subNavigation->getter() as $v) {
						...
						$delHide = "";
						if(in_array($v['idx'], $RESTRICTED_PAGE_DELETE_IDX)){
							$delHide = "style='visibility:hidden'";
						}


						$nav .= "<div class=\"row level-2 sub-".$v['cid']."\" data-item=\"".$v['idx']."\" data-cid=\"".$v['cid']."\" data-level=\"2\">";
						$nav .= "<div class=\"cell roboto-font\">".$v['idx']."</div>";
						$nav .= "<div class=\"cell\"><a href=\"#\">".$string->cut($v['title'], 20)."</a></div>";
						...
						$nav .= "<a href=\"javascript:void(0)\" onclick=\"changeVisibility('".$v["visibility"]."','".$v["idx"]."')\">
					 	...
					 	// $nav .= "<a href=\"".$aditionalLink.$v["idx"]."\">
					 	// 	<i class=\"material-icons tooltipped\" data-position=\"bottom\" data-delay=\"50\" data-tooltip=\"დამატებითი ველები\">build</i>
					 	// </a>";

					 	$nav .= $usefull_type2;

					 	$nav .= "<a href=\"javascript:void(0)\" onclick=\"askRemovePage('0','".$v['position']."','".$v['idx']."', '".$v["cid"]."')\" ".$delHide.">
							<i class=\"material-icons tooltipped\" data-position=\"bottom\" data-delay=\"50\" data-tooltip=\"წაშლა\">delete</i>
					 	</a>";
						...
						if($subSubNavigation->getter()){
							foreach ($subSubNavigation->getter() as $v3) {
								...
								$delHide = "";
								if(in_array($v3['idx'], $RESTRICTED_PAGE_DELETE_IDX)){
									$delHide = "style='visibility:hidden'";
								}
								...
								$nav .= "<div class=\"row level-3 sub-".$v3['cid']."\" data-item=\"".$v3['idx']."\" data-cid=\"".$v3['cid']."\" data-level=\"3\">";
								...
								// $nav .= "<a href=\"".$aditionalLink.$v3["idx"]."\">
								// <i class=\"material-icons tooltipped\" data-position=\"bottom\" data-delay=\"50\" data-tooltip=\"დამატებითი ველები\">build</i>
								// </a>";

					 			$nav .= $usefull_type3;

					 			$nav .= "<a href=\"javascript:void(0)\" onclick=\"askRemovePage('0','".$v3['position']."','".$v3['idx']."', '".$v3["cid"]."')\" {$delHide}>
								<i class=\"material-icons tooltipped\" data-position=\"bottom\" data-delay=\"50\" data-tooltip=\"წაშლა\">delete</i>
					 			</a>";
								...
								if($subSubNavigation2->getter()){
									foreach ($subSubNavigation2->getter() as $v4) {
										$delHide = "";
										if(in_array($v4['idx'], $RESTRICTED_PAGE_DELETE_IDX)){
											$delHide = "style='visibility:hidden'";
										}
										...
										$usefull_url4 = "<a href=\"".$usefull_url4."\">";
										...
										$nav .= "<div class=\"row level-4 sub-".$v4['cid']."\" data-item=\"".$v4['idx']."\" data-cid=\"".$v4['cid']."\" data-level=\"4\">";
										$nav .= "<div class=\"cell roboto-font\">".$v4['idx']."</div>";
										
										$nav .= "<div class=\"cell\"><a href=\"#\">".$string->cut($v4['title'],20)."</a></div>";
										...
										$nav .= "<a href=\"javascript:void(0)\" onclick=\"changeVisibility('".$v4["visibility"]."','".$v4["idx"]."')\">
							 			...
							 			// $nav .= "<a href=\"".$aditionalLink.$v4["idx"]."\">
									 	// 	<i class=\"material-icons tooltipped\" data-position=\"bottom\" data-delay=\"50\" data-tooltip=\"დამატებითი ველები\">build</i>
									 	// </a>";

							 			$nav .= $usefull_url4;

							 			$nav .= "<a href=\"javascript:void(0)\" onclick=\"askRemovePage('0','".$v4['position']."','".$v4['idx']."', '".$v4["cid"]."')\" {$delHide}>
										<i class=\"material-icons tooltipped\" data-position=\"bottom\" data-delay=\"50\" data-tooltip=\"წაშლა\">delete</i>
							 			</a>";
